...
public class UDTGenerator<T> implements net.java.quickcheck.Generator<T>
{
    ...
    protected void initInnerGenerators()
    ...
    protected void initSubtypeGenerators()
        throws InstantiationException, IllegalAccessException
    {
        if (!this.allSubtypeGeneratorsInitialized)
        {
            ...
            Set<Class> subtypes =
                Utils.getConcreteDescendentsInSamePackage(this.type);
            for (Class<?> c : subtypes)
            {
                try
                {
                    Constructor<?> ctor =
                        UDTGenerator.class.getDeclaredConstructor(Class.class);
                    Reflector.ensureIsAccessible(ctor);

                    UDTGenerator<?> cGen =
                        (UDTGenerator<?>)ctor.newInstance(c);
    ...
    protected void initFieldGenerators()
        throws InstantiationException, IllegalAccessException
    {
        if (!this.allFieldGeneratorsInitialized)
        {
            ...
            T genObj = (T)Utils.getDummyObject(this.type);
            // // throw-away
            // object
                ...
                else
                {
                    try
                    {
                        ...
                        UDTGenerator<?> fieldGen =
                            (UDTGenerator<?>)ctor.newInstance(fieldType);
